...
export function meta({ params }: Route.MetaArgs) {
	...
	const metaData = {
		...
		en: {
			title: "FAQ - TmpMail Temporary Email Service Guide & Troubleshooting",
			description:
				"Comprehensive FAQ for TmpMail temporary email: How to use disposable email, email retention time, attachment downloads, privacy security, etc. Quickly find answers for using temporary email service.",
		},
		ja: {
			title:
				"よくある質問 - TmpMail一時メールサービスガイド＆トラブルシューティング",
			description:
				"TmpMail一時メールの包括的なFAQ：使い捨てメールの使用方法、メール保存期間、添付ファイルのダウンロード、プライバシーセキュリティなど。一時メールサービスの使用に関する回答を素早く見つけられます。",
		},
	};
...
export default function LangFAQ() {
	...
	useEffect(() => {
		console.log("[FAQ Debug] useEffect triggered:", {
			lang,
			currentLang: getCurrentLanguage(),
			isSupported: isSupportedLanguage(lang || ""),
		});
		...
		if (lang !== getCurrentLanguage()) {
			console.log(
				"[FAQ Debug] Language change needed:",
				lang,
				"->",
				getCurrentLanguage(),
			);
